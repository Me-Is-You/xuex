...
import { BookOpen, Target, TrendingUp, Award, ArrowRight, BrainCircuit, Zap, ShieldCheck, Video, Network } from 'lucide-react';

export default function LandingPage() {
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans">
      {/* Navigation */}
      <nav className="flex items-center justify-between px-8 py-6 bg-white/80 backdrop-blur-md sticky top-0 z-50 border-b border-emerald-50">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm font-medium text-slate-600">
          <Link href="#features" className="hover:text-emerald-600 transition-colors">备考方案</Link>
          <Link href="#roadmap" className="hover:text-emerald-600 transition-colors">学习路径</Link>
          <Link href="/dashboard" className="hover:text-emerald-600 transition-colors">我的控制台</Link>
        </div>
        <Link 
          href="/dashboard" 
          className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-full text-sm font-semibold transition-all shadow-lg shadow-emerald-200"
        >
          立即开始
        </Link>
      </nav>
      ...
      <header className="relative px-8 pt-20 pb-32 overflow-hidden">
        <div className="max-w-6xl mx-auto text-center relative z-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 text-xs font-bold mb-6 border border-emerald-100 uppercase tracking-wider">
            <Zap className="w-3 h-3" /> 2027 届考生专属平台
          </div>
          <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight mb-8 leading-tight">
            智能化备考 <br />
            <span className="text-emerald-600">决胜 2027 陕西专升本</span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <Link 
              href="#roadmap" 
              className="flex items-center gap-2 bg-white border border-slate-200 hover:border-emerald-300 text-slate-700 px-8 py-4 rounded-xl text-lg font-bold transition-all w-full sm:w-auto justify-center"
            >
              查看学习路线
            </Link>
          </div>
        ...
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full pointer-events-none -z-10 opacity-30">
          <div className="absolute top-20 left-1/4 w-72 h-72 bg-emerald-200 rounded-full mix-blend-multiply filter blur-3xl animate-blob"></div>
          <div className="absolute top-40 right-1/4 w-72 h-72 bg-teal-200 rounded-full mix-blend-multiply filter blur-3xl animate-blob animation-delay-2000"></div>
          <div className="absolute -bottom-8 left-1/3 w-72 h-72 bg-green-200 rounded-full mix-blend-multiply filter blur-3xl animate-blob animation-delay-4000"></div>
        </div>
      </header>
